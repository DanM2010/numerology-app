...
def determine_users_birth_path(dob)
	...
	(0..dob.length.to_i-1).each do |i|
		birth_path += dob[i].to_i
	end
	while birth_path > 9
		number_of_digits = birth_path.to_s.length.to_i
		tmp_str = birth_path.to_s
		birth_path = 0
		(0..number_of_digits-1).each do |i|
			birth_path += tmp_str[i].to_i
		end
	end
	return birth_path
end

def determine_birth_message(birth_path_number)	
